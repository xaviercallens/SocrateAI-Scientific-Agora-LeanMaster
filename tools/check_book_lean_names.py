...
instance axioms propext Quot.sound Classical.choice True False true false Prop Type Sort lean_lib push_neg""".split())
...
def candidates(t):
    ...
    for r in raw:
        for x in re.split(r"[,\s{}()]+", r.replace("\\_", "_")):
            x = x.strip(".:;`")
            if (len(x) < 3 or x.startswith("_") or x in TACTICS or not re.fullmatch(r"[A-Za-z][\w'.₀-₉]*", x)
...
unresolved = sorted({n for s in per_file.values() for n in s if n not in full and n not in tails})
...
unknown = {n for i, n in enumerate(unresolved, start=4) if i in bad_lines}
...
unknown = {n for n in unknown if n not in prefixes}
missing = 0
for f in files:
