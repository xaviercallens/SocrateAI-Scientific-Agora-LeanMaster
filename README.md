...
every theorem and lemma (**817 audited theorems across the ten first-party libraries** depend on
...
| **Kernel Verification** | None (paper only) | 817/817 audited theorems across ten libraries: 0 sorry, standard axioms only | Tier A |
...
```
Last full run (2026-09-19, all ten libraries re-audited on Lean v4.34.0-rc2 during the toolchain migration (§0b of `docs/VERIFIED_FOUNDATION.md`); `DualScaleStream2` re-audited 2026-09-20 after v3.37.0, `DualScaleDyons` after v3.42.0), **817 theorems audited across all ten libraries, 0 failing**:
...
| `DualScaleDyons` (Streams 5, 8, 9 bridge) | 147 | 0 |
| **Total** | **817** | **0** |
...
not say so until now. **Excluding them, 814 declarations carry mathematical content.** A fourth vacuous statement,
