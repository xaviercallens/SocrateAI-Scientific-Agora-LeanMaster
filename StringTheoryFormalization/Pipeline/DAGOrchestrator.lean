...
def formalizationDAG : List DAGNode := [
  ...
  { id := "M1",  name := "FractionalSobolev",  status := .Verified, phase := none,
    dependencies := ["F1"],      sorryCount := 0 },
  { id := "M2",  name := "FourierMultipliers", status := .Verified, phase := none,
    dependencies := ["M1"],      sorryCount := 0 },
  { id := "M3",  name := "MildPDEs",           status := .Verified, phase := none,
    dependencies := ["M1"],      sorryCount := 0 },
  { id := "M4",  name := "EnergyBounds",       status := .Verified, phase := none,
    dependencies := ["M1","M2"], sorryCount := 0 },
  { id := "WS4", name := "VertexOperators",    status := .Verified, phase := none,
    dependencies := ["F1","M2"], sorryCount := 0 },
  { id := "WS5", name := "PicardSpectral",     status := .Verified, phase := none,
    dependencies := ["WS4"],     sorryCount := 0 },
  ...
  { id := "WS14",name := "InvariantLocks",     status := .Verified, phase := none,
    dependencies := ["WS13"],    sorryCount := 0 },
  { id := "WS15",name := "StiffIntegrators",   status := .Verified, phase := none,
    dependencies := ["M3","WS14"],sorryCount := 0 },
  { id := "WS16",name := "SwamplandSafe",      status := .Verified, phase := none,
    dependencies := ["M4","WS9"],sorryCount := 0 },
  ...
  { id := "WS19",name := "TDAMapper",          status := .Verified, phase := none,
    dependencies := ["F1"],      sorryCount := 0 },
  -- ── FRONTIER BLOCKS (6) ───────────────────────────────────────────────
  { id := "FR1", name := "CentralCharge",      status := .Verified, phase := none,
    dependencies := ["M2","WS4"],sorryCount := 0 },
  { id := "FR2", name := "ChiralPrimaries",    status := .Verified, phase := none,
    dependencies := ["FR1"],     sorryCount := 0 },
  { id := "FR3", name := "SL2CSymmetry",       status := .Verified, phase := none,
    dependencies := ["M2","WS4","FR1"],sorryCount := 0 },
  { id := "FR4", name := "HodgeNumbers",       status := .Verified, phase := none,
    dependencies := ["WS10","WS6","FR2"],sorryCount := 0 },
  { id := "FR5", name := "FTermPotential",     status := .Verified, phase := none,
    dependencies := ["WS10","WS7","FR4"],sorryCount := 0 },
  { id := "FR6", name := "ModuliGeodesics",    status := .Verified, phase := none,
    dependencies := ["WS14","FR4","FR5"],sorryCount := 0 },
  -- ── PIPELINE MODULES ──────────────────────────────────────────────────
  { id := "P1",  name := "DAGOrchestrator",    status := .Verified, phase := none,
    dependencies := [],          sorryCount := 0 },
  { id := "P2",  name := "TacticSearch",       status := .Verified, phase := none,
