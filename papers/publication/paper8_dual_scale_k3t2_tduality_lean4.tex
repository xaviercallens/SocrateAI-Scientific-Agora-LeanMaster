...
of the Eguchi--Ooguri--Tachikawa Mathieu-moonshine coefficients. All 99 theorems are accepted by
...
stability,'' ``formalizes string theory,'' ``first fully certified,'' or ``all of physics.'' The
...
$\mathcal{D}(\mathbf 1_d)=2d$ (\texttt{dualScale\_one}); \textbf{the Lean statement proves the
lower bound and its attainment at $G=1$, not that $G=1$ is the unique minimizer} --- uniqueness of
the minimizing metric is not claimed or proved here, and this paper does not write ``equality
iff'' for this theorem. At $d=1$, $G=R^2$,
\begin{equation}
...
\texttt{dualScale\_one} & A --- bound attained at $G=\mathbf 1_d$ (attainment only, not uniqueness) \\
...
\begin{scopebox}
\textbf{Is proved (Tier A, kernel-checked, 99 theorems, 0 \texttt{sorry}, standard axioms only):}
...
one of which was checked only symbolically (sympy) and is explicitly flagged as not Tier~A; that
$G=1$ is the \emph{unique} minimizer of the dual-scale bound; invariance of the full string
...
project's 99 final theorems --- are reported \emph{per package}, not as a global tier table; the
...
\end{center}
reports \textbf{99 theorems}, \textbf{0 failing}, with every declaration's axiom footprint limited
