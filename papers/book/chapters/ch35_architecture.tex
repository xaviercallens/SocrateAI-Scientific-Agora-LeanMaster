...
order: a type of $d\times d$ real matrices with a genuine inverse (Mathlib's
\lean{Matrix.nonsing\_inv}), block matrices (\lean{Matrix.fromBlocks}), the invertibility
...
\lean{Statistical}\allowbreak\lean{LearningBridge} \newline\source{ll.~46--81} &
...
\lean{atlas\_k3\_euler\_}\allowbreak\lean{characteristic}, \lean{atlas\_k3\_signature},
\lean{atlas\_poincare\_}\allowbreak\lean{curvature\_negative} &
...
\item If the probe itself fails to elaborate---typically because the library was not
  rebuilt---exit with code 2 and the message \lean{AUDIT ERROR} (ll.~70--77). This clause
...
\lean{CHANGED} and exit 1. If instead the agent edited \lean{Tier.toNat} so that
\lean{Tier.A} mapped to $3$ and \lean{Tier.B} to $4$, no theorem statement would change,
but the definition's hash (\lean{d839f1061e526f25} on the current text) would, and the
...
\begin{exercise}[$\star\star$]
For each edit, say whether \lean{statement\_lock.py --check} would report \lean{CHANGED},
