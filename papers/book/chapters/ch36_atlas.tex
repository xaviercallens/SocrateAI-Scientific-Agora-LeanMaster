...
$4$ O7-planes of charge $-16$ \source{Moonshine/KummerTadpole.lean, ll.~96--126}; in
\lean{d7\_tadpole\_cancellation} of the Foundation library it was, in the snapshot the atlas
was computed from (2026-09-17), $32$ D7-branes of charge $+2$ against $16$ O7$^-$ fixed points
of charge $-4$. Both gave $64-64=0$, but the second took the count of the $T^4/\Z_2$ fixed
points and the charge of an O7-plane, a combination that matches no orientifold; it was
corrected on 2026-09-18 to the $4$ O7-planes and $16$ D7-branes of \eqref{eq:tadpole:d7}
(\cref{sec:tadpole:lean}). The first is a rescaling by~$4$ of the same total. The fourth, \lean{tadpole\_cancellation} of
...
\source{StringDynamics/TadpoleConstraint.lean, ll.~92--99}. A reader trusting docstrings
alone would count four proofs of tadpole cancellation; the atlas plus the source reveal
two integer bookkeepings (after the correction, related by a factor~$4$) and one rearrangement of an assumption. The physics,
...
lattice a Gram matrix in \texttt{Lattice.Mukai}; for the tadpole the corrected Foundation file,
which contains Polchinski's whole table (\lean{oplane\_total\_independent}). Then replace each duplicate by a corollary that \emph{states its
...
{\tabcolsep=2.5pt\def\_{\textunderscore\allowbreak}\input{generated/atlas_tables}}
