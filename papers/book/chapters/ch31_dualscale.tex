...
\item[(A)] \textbf{Mathematics.} For every $R>0$, $R_{\rm eff}(\ap/R)=R_{\rm eff}(R)$ and
 ...
 \TA{} (\cref{sec:dualscale:lean}); the uniqueness statements are proved by hand below and not
 formalized.
...
Mathlib, and \cref{sec:dualscale:lean} shows the proof assembled from them. The uniqueness
paragraph is \emph{not} part of the formalized statement: Lean proves the inequality and,
separately, that the value $2d$ is attained at $G=\mathbf1$.
...
\midrule
uniqueness of the minimizer; $B$-shifts increase $\Tr\HH$; the general $\mathrm{Sym}^2$ lemma &
...
\item Lean (\TA, all $d$, over $\R$): \lean{dualScale\_eq}, \lean{dualScale\_inv},
 ...
 \lean{dualScale\_circle}, \lean{circle\_effective\_scale\_ge\_two} (direct proof). Not
 formalized: uniqueness of the minimizer. Older modules are arithmetic shadows over
