...
$\dd\alpha=0$ on a line; the model with \lean{alpha := 2 * (X.v * Y.alpha)} would satisfy both
...
theorem dorfman_antisymm_three (X Y : CourantSection) :
  ...
  sorry
\end{leancode}
(b) Define \lean{DorfmanExp X Y} with form slot \lean{2 * (X.v * Y.alpha)} and vector slot $0$, and
