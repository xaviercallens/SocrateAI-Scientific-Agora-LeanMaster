...
$-2^{p-5}$, and the vanishing of the total are convention independent. The Lean libraries use
two bookkeepings of this cancellation (\cref{sec:tadpole:lean}): the units of
\cref{tab:tadpole:oplanes}, and the same with all charges multiplied by~$4$. Both have the ratio
$-4$ at $p=7$. A third bookkeeping, with sixteen ``O7-planes'' at the fixed points of $T^4/\Z_2$
and ratio $-2$, matched no row of the table; it was corrected on 2026-09-18.
\end{pitfallbox}
...
of the $-24$. The remaining $-8$ is carried by the four O7-planes, $-2$ each: TT state that
``2 units of D3 brane charge are induced on the world volume of each O7-plane and one units of
D3 brane charge in each D7-brane, giving rise to a total of 24 units'', cancelled as
$\frac12N_{\rm flux}+N_{D3}=24$ \source{TT §2.2, eq.~(2.3), ll.~164--171} (their sign
convention is opposite to GKP's). The two routes to $24$, $4\cdot2+16\cdot1$ and
$\chi(K3\times K3)/24$, agree; the arithmetic is \lean{induced\_d3\_charge\_matches\_target}.
\end{remark}
...
-- StringTheory.Foundation.StringTheory.TadpoleCancellation (corrected 2026-09-18)
theorem d7_tadpole_cancellation : totalD7Charge + totalO7Charge = 0 := by rfl
theorem plane_brane_ratio : chargeO7Minus = -(2 ^ (7 - 5) : Int) * chargeD7Brane := by rfl
theorem oplane_total_independent :
    ∀ k < 7, (planes (k + 3) : Int) * planeCharge (k + 3) + 16 * 4 = 0 := by decide
theorem induced_d3_charge_matches_target :
    (numO7Planes : Int) * d3ChargePerO7 + (numD7Branes : Int) * d3ChargePerD7
      = d3TadpoleTarget := by rfl
...
\lean{d7\_tadpole\_cancellation} unfolds to $16\cdot1+4\cdot(-4)=0$, exactly
\eqref{eq:tadpole:d7}, and both \lean{rr\_tadpole\_cancellation} to $16\cdot4+4\cdot(-16)=0$,
the same row with all charges multiplied by~$4$ (\lean{o7\_d7\_ratio} checks that the ratio is
still $-4$); all are proved by \lean{rfl}. \lean{oplane\_total\_independent} is the whole of
\cref{tab:tadpole:oplanes} in units of $\mu_p/4$, for $p=3,\dots,9$, checked by \lean{decide}.
\lean{induced\_d3\_charge\_matches\_target} is the arithmetic of \cref{rem:tadpole:where}:
$4\cdot2+16\cdot1=24\cdot24/24$.

\emph{A correction.} Until 2026-09-18 the Foundation file placed sixteen O7$^-$-planes at the
fixed points of $T^4/\Z_2$, with $32$ D7-branes of charge $+2$, and stated the D3 target as
$\chi(K3)/24=1$. Sixteen planes belong to $p=5$ in \cref{tab:tadpole:oplanes}, the ratio $-2$
matched no row, and the orientifold of $\KT/\Z_2$ acts on the $T^2$, with four fixed points
\source{TT §2.2, ll.~160--163; Sen l.~242}; the sixteen fixed points of $T^4/\Z_2$ are the
Kummer limit of the K3 factor (\cref{ch:kummer}). The D3 target is $\chi(K3\times K3)/24=24$;
$\chi(K3)/24=1$ is the charge of a single wrapped D7-brane. A literature check by a sibling
project flagged both points; they were verified against the pinned sources before the file
was changed. The first edition of this chapter already noted that ``the label `O7' in that
file does not match its numbers''. Finally,
\begin{leancode}
...
\item \lean{rr\_tadpole\_cancellation} (in \lean{UseCase3\_FrontierTriad}) and
...
$16-16=0$ and $64-64=0$ (ratio $-4$); Polchinski's table for $3\le p\le9$; $4\cdot2+16\cdot1=24$ & \TA & \lean{d7\_tadpole\_cancellation}, \lean{rr\_tadpole\_cancellation},\newline
 \lean{oplane\_total\_independent}, \lean{induced\_d3\_charge\_matches\_target}\\
...
The library proves \lean{oplane\_total\_independent} by \lean{decide}, one \lean{p} at a time.
Prove instead, for all \lean{p} with \lean{3 ≤ p} and \lean{p ≤ 9} and without case analysis,
that \lean{(planes p : ℤ) * planeCharge p = -64}, using
\lean{planes p := 2 \^{} (9 - p)}, \lean{planeCharge p := -(2 \^{} (p - 3) : ℤ)} and
\lean{pow\_add}. Which of the two bookkeepings of \cref{sec:tadpole:lean} does the $p=7$ case
reproduce, and which row would the withdrawn ``sixteen O7-planes'' bookkeeping have needed?
\end{exercise}
